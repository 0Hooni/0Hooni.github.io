...
tags:
  ...
  - Git
---
최근에 패키지를 설치하거나 제거하는 일이 몇번 있었는데, 그럴때마다 `Package Resolution Failed`에러가 지속적으로 발생했다.

![](assets/img/post/2025/04_07_Package_Resolution_Failed.png)_나한테 왜그래~~_

저기서 `Add Anyway`를 선택하면 어떻게든 설치는 되는데 

문제는 몇몇 패키지의 경우 선택해서 설치할 수 있고 
...
![](assets/img/post/2025/04_07_스택오버플로우_베댓.png){: w="400"}_스택 오버플로우 베스트 댓글_
...
위 코드와 같이 전체 디렉토리를 안전하게 만들어줄 수 있고 
...
우리가 SPM을 설치할 때 `https://github.com/SPM.git`과 같은 형태로 SPM을 받아오는데, 이게 해당 레포지토리를 **Bare repository**형태로 가져온다는 의미였다.
